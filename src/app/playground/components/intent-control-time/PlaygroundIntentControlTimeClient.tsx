...
// src/app/playground/components/intent-control-time/PlaygroundIntentControlTimeClient.tsx
// PlaygroundIntentControlTimeClient
// - Uses PlaygroundComponentShell to test IntentControlTime
// - Demonstrates both standalone mode and IntentControlField wrapper mode
// - Covers timeMode=single/split + singleVariant=native/text
// - Adds constraints (min/max), readOnly, invalid, splitPart, etc.
...
type SplitPart = "all" | "hour" | "minute";
...
export default function PlaygroundIntentControlTimeClient() {
    ...
    // Local props
    const [size, setSize] = useState<TimeSize>("md");
    const [fullWidth, setFullWidth] = useState(true);

    const [timeMode, setTimeMode] = useState<TimeMode>("split");
    ...
    const [max, setMax] = useState("19:30");

    // Split labels
    const [hourLabel, setHourLabel] = useState("Heure");
    const [minuteLabel, setMinuteLabel] = useState("Minute");

    // Field wrapper options
    const [fieldLabel, setFieldLabel] = useState("Heure");
    const [fieldHint, setFieldHint] = useState(
        "Saisis une heure valide. En split: tab, backspace et paste sont gérés."
    );
    const [fieldError, setFieldError] = useState("Heure invalide");
    ...
    const resolvedWithWarnings = useMemo(() => {
        ...
        // Here we pass the DS props + a "representative" time state.
    ...
    const controlsLocal = (
        <>
            ...
            <SelectRow label="Size">
                <Select
                    value={size}
                    onChange={(v) => setSize(v as TimeSize)}
                    options={["xs", "sm", "md", "lg", "xl"]}
                />
            </SelectRow>

            <SelectRow label="Time mode">
                ...
                <div className="mt-2 text-[11px] opacity-40">
                    ...
                    <span className="font-mono">9h30</span> ou{" "}
                    <span className="font-mono">0930</span>.
                </div>
            </SelectRow>
            ...
            <SelectRow label="Constraints (min/max)">
                ...
                {useMinMax ? (
                    <div className="mt-3 grid grid-cols-1 gap-3">
                        <div>
                            <div className="text-[11px] opacity-55 mb-1">min (HH:MM)</div>
                            <TextInput value={min} onChange={setMin} placeholder="08:00" />
                        </div>
                        <div>
                            <div className="text-[11px] opacity-55 mb-1">max (HH:MM)</div>
                            <TextInput value={max} onChange={setMax} placeholder="19:30" />
                        </div>
                    </div>
                ) : (
                    <div className="mt-2 text-[11px] opacity-40">
                        Active pour tester le comportement navigateur en{" "}
                        <span className="font-mono">type=time</span>.
                    </div>
                )}
            </SelectRow>
            ...
            {timeMode === "split" ? (
                <SelectRow label="Split labels (a11y)">
                    <div className="grid grid-cols-1 gap-3">
                        <div>
                            <div className="text-[11px] opacity-55 mb-1">hourLabel</div>
                            <TextInput value={hourLabel} onChange={setHourLabel} />
                        </div>
                        <div>
                            <div className="text-[11px] opacity-55 mb-1">minuteLabel</div>
                            <TextInput value={minuteLabel} onChange={setMinuteLabel} />
                        </div>
                    </div>
                </SelectRow>
            ) : null}
    ...
    const codeString = useMemo(() => {
        ...
        const minMaxLines = useMinMax ? `      min="${min}"\n      max="${max}"\n` : "";
...
${
    timeMode === "single"
      ...
      hourLabel="${hourLabel.replaceAll('"', '\\"')}"
      minuteLabel="${minuteLabel.replaceAll('"', '\\"')}"
`
...
`;
        ...
        const standaloneSlots = !wrapInField
            ? `      leading={${withLeading ? "<span aria-hidden>⏱️</span>" : "undefined"}}
...
import { IntentControlTime, IntentControlField } from "intent-design-system";
...
${baseProps}${standaloneSlots}    />`
    ...
    const preview = useMemo(() => {
        const timeEl = (
            <IntentControlTime
                ...
                value={value}
                onValueChange={(next) => setValue(next)}
                leading={!wrapInField && withLeading ? <span aria-hidden>⏱️</span> : undefined}
                trailing={!wrapInField && withTrailing ? <span aria-hidden>⌘</span> : undefined}
            />
        );

        if (!wrapInField) return timeEl;
        ...
        return (
            <IntentControlField
                ...
                invalid={invalid}
                disabled={disabled}
                labelFor="ids-time-demo"
            >
                {React.cloneElement(timeEl as any, {
                    id: "ids-time-demo",
                    insideField: true,
                    leading: undefined,
                    trailing: undefined,
    ...
    return (
        <PlaygroundComponentShell
            identity={IntentControlTimeIdentity}
            propsTable={IntentControlTimePropsTable}
            locale="fr"
            ...
            renderPreview={() => (
                <div className="w-full min-w-0">
                    ...
                    <div className="mt-3 text-xs opacity-70">
                        mode=<span className="font-mono">{previewMode}</span>, timeMode=
                        <span className="font-mono"> {timeMode}</span>
                        {timeMode === "split" ? (
                            <>
                        ...
                        {timeMode === "single" ? (
                            <>
                                , singleVariant=<span className="font-mono"> {singleVariant}</span>
                            </>
                        ) : null}
                        , variant=<span className="font-mono"> {variant}</span>, intent=
                    ...
                    <div className="mt-2 text-[11px] opacity-55">
                        ...
                        <span className="font-mono">09:30</span>,{" "}
                        <span className="font-mono">9h30</span> ou{" "}
                        <span className="font-mono">0930</span> pour valider le parse.
                    </div>
                </div>
            )}
        />
