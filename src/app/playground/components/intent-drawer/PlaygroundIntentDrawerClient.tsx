...
// src/app/playground/components/intent-drawer/PlaygroundIntentDrawerClient.tsx
// PlaygroundIntentDrawerClient
// - Uses PlaygroundComponentShell to test IntentDrawer
// - Uses DS exports: Identity + PropsTable
// - Includes sample content + footer + selection log
// - Exercises: positions, sizes, overlay, opaqueBackdrop, focus trap, scroll lock, preventClose

import React, { useMemo, useState } from "react";

import {
    IntentDrawer,
    type IntentDrawerPosition,
    type IntentDrawerSize,
    ...
    IntentDrawerIdentity,
    IntentDrawerPropsTable,
...
export default function PlaygroundIntentDrawerClient() {
    ...
    const [position, setPosition] = useState<IntentDrawerPosition>("right");
    const [size, setSize] = useState<IntentDrawerSize>("md");
    const [overlay, setOverlay] = useState(true);
    const [opaqueBackdrop, setOpaqueBackdrop] = useState(false); // ✅ NEW
    ...
    const [title, setTitle] = useState("🧭 Drawer of Intent");
    const [description, setDescription] = useState(
        "Un panneau glissant pour navigation, filtres, settings…"
    );

    const [sizePx, setSizePx] = useState<string>(""); // numeric override
    ...
    const sizePxNumber = useMemo(() => {
        const raw = sizePx.trim();
    ...
    const footer = useMemo(() => {
        return (
            <div className="flex items-center justify-between text-xs opacity-70">
                ...
                <div className="opacity-60">pos: {position}</div>
            </div>
        );
    }, [position]);
        ...
        <>
            ...
            <SelectRow label="Layout">
                <div className="space-y-2">
                    <Select
                        value={position}
                        onChange={(v) => setPosition(v as IntentDrawerPosition)}
                        options={["right", "left", "top", "bottom"]}
                    />
                    <Select
                        value={size}
                        onChange={(v) => setSize(v as IntentDrawerSize)}
                        options={["xs", "sm", "md", "lg", "xl"]}
                    />
                    <NumberInput
                        value={sizePx}
                        onChange={setSizePx}
                        placeholder="sizePx (override) ex: 520"
                    />
                    ...
                    <div className="space-y-2 pt-2">
                        ...
                        />
                        <CheckboxRow label="inset" checked={inset} onChange={setInset} />
                    </div>
                </div>

                <div className="mt-2 text-[11px] opacity-45">
                    sizePx override:{" "}
                    <span className="font-mono">{sizePxNumber ? `${sizePxNumber}px` : "—"}</span>
                </div>
            </SelectRow>
    ...
    const codeString = useMemo(() => {
        ...
        const sizePxLine = sizePxNumber ? `    sizePx={${sizePxNumber}}\n` : "";

        return `import { IntentDrawer } from "intent-design-system";
...
${sizePxLine}      overlay={${overlay}}
    ...
    >
      <div style={{ padding: 12 }}>Hello drawer ✨</div>
    </IntentDrawer>
  );
}`;
    }, [
        ...
        <div className="space-y-4">
            <div className="rounded-2xl bg-black/25 ring-1 ring-white/10 p-4">
                <div className="text-sm opacity-90">
                    Ici, tu peux mettre un menu, des filtres, un formulaire, une mini-map… 🧩
                </div>
                <div className="mt-2 text-xs opacity-60">
                    Astuce: teste <span className="font-mono">Tab</span> et{" "}
                    <span className="font-mono">Shift+Tab</span> si focus trap activé.
                </div>
                <div className="mt-2 text-xs opacity-60">
                    opaqueBackdrop:{" "}
            ...
            <div className="space-y-2">
                ...
                <button
                    ...
                    onClick={() => {
                        const line = `${new Date().toLocaleTimeString()} · Save`;
                ...
                <button
                    ...
                    onClick={() => {
                        const line = `${new Date().toLocaleTimeString()} · Reset`;
                        setActionLog((prev) => [line, ...prev].slice(0, 8));
                        setNotes("");
                    }}
                >
    ...
    return (
        <PlaygroundComponentShell
            identity={IntentDrawerIdentity}
            propsTable={IntentDrawerPropsTable}
            locale="fr"
                ...
                <div className="w-full min-w-0">
                    <div className="flex items-center gap-3">
                        ...
                        <div className="text-xs opacity-55">
                            ...
                            <span className="opacity-40">·</span> size:{" "}
                            <span className="font-mono">
                                {sizePxNumber ? `${sizePxNumber}px` : size}
                            </span>{" "}
                    ...
                    <IntentDrawer
                        ...
                        position={position}
                        size={size}
                        {...(sizePxNumber !== undefined ? { sizePx: sizePxNumber } : {})}
                        overlay={overlay}
                        opaqueBackdrop={opaqueBackdrop} // ✅ NEW
                        inset={inset}
                        closeOnOverlay={closeOnOverlay}
                        closeOnEscape={closeOnEscape}
                        lockScroll={lockScroll}
